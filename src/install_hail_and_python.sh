...
do
   echo '#####################################################################################'
   scp -i ~/.ssh/id_rsa/${KEY} ${HOME}/install_python.sh hadoop@${WORKERIP}:/tmp/install_python.sh
   ssh -i ~/.ssh/id_rsa/${KEY} hadoop@${WORKERIP} 'export PATH=/usr/local/bin:$PATH'
   ssh -i ~/.ssh/id_rsa/${KEY} hadoop@${WORKERIP} "sudo ls -al /tmp/install_python.sh"
   ssh -i ~/.ssh/id_rsa/${KEY} hadoop@${WORKERIP} "sudo /tmp/install_python.sh &"  
done
